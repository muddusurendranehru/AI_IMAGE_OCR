  ...
  try {
    ...
    reportsResult.rows.forEach((report, index) => {
      ...
      if (report.extracted_data) {
        ...
        if (data.homaIQScore) {
          console.log(
            `      HOMA-IQ Score: ${data.homaIQScore.score} (${data.homaIQScore.riskLevel})`
          );
        }
        ...
        if (data.healthMetrics) {
          ...
          if (hm.tygIndex)
            console.log(`      TYG Index: ${hm.tygIndex.value} (${hm.tygIndex.status})`);
          if (hm.bmi) console.log(`      BMI: ${hm.bmi.value} (${hm.bmi.status})`);
          if (hm.waistCircumference)
            console.log(
              `      Waist: ${hm.waistCircumference.value} cm (${hm.waistCircumference.status})`
            );
        }
