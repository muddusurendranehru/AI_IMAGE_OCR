...
const uploadLabReport = async (req, res) => {
  try {
    ...
    if (
      ...
      Object.keys(ocrResult.extractedData.labValues).length > 0
    ) {
      ...
      if (healthMetrics.tygIndex) {
        console.log(
          `📈 TYG Index: ${healthMetrics.tygIndex.value} (${healthMetrics.tygIndex.status})`
        );
      }
      ...
      if (healthMetrics.waistCircumference) {
        console.log(
          `📈 Waist: ${healthMetrics.waistCircumference.value} cm (${healthMetrics.waistCircumference.status})`
        );
      }
    }
...
const getAllReports = async (req, res) => {
  try {
    ...
    const countResult = await db.query('SELECT COUNT(*) FROM lab_reports WHERE uploaded_by = $1', [
      userId
    ]);
...
const deleteReport = async (req, res) => {
  try {
    ...
    const reportResult = await db.query('SELECT image_path FROM lab_reports WHERE id = $1', [id]);
...
const batchUploadLabReports = async (req, res) => {
  try {
    ...
    console.log(`📚 Batch: ${files.length} files for ${patientName} (Patient ID: ${patientId})`);

    // Check file limit
    if (files.length > 30) {
      return res.status(400).json({
        success: false,
        error: `Too many files. Maximum 30 files allowed. You uploaded ${files.length} files.`
      });
    }
    ...
    // Process each file with error handling and timeout protection
    for (let i = 0; i < files.length; i++) {
      try {
        console.log(`📄 Processing ${i + 1}/${files.length}: ${files[i].originalname}`);

        // Add per-file timeout (30 seconds per file)
        const fileTimeout = new Promise((_, reject) => {
          setTimeout(() => reject(new Error('File processing timeout')), 30000);
        });

        const ocrPromise = ocrService.processLabReport(files[i].path);
        const ocrResult = await Promise.race([ocrPromise, fileTimeout]);
        ...
        if (ocrResult.success) {
          ...
          console.warn(`⚠️ Failed to process ${files[i].originalname}: ${ocrResult.error}`);
          failedCount++;
        }
      } catch (fileError) {
        if (fileError.message === 'File processing timeout') {
          console.error(`⏱️ Timeout processing ${files[i].originalname} (30s limit)`);
        } else {
          console.error(`❌ Error processing ${files[i].originalname}:`, fileError.message);
        }
        failedCount++;
      }
    }

    console.log(`✅ Processed: ${processedCount}/${files.length} files successfully`);
    if (failedCount > 0) {
      console.warn(`⚠️ Failed: ${failedCount} files`);
    }

    // Calculate scores if we have lab values
    let homaIqResult = null;
    let healthMetrics = null;
    ...
    console.log('💾 Saving batch report to database...');
    ...
    // Always send response, even if some files failed
    res.json({
      success: true,
      report: savedReport,
      ...
      message: `Successfully processed ${processedCount} out of ${files.length} files`
    });
  } catch (error) {
    ...
    res.status(500).json({
      success: false,
      error: error.message,
      details: 'Batch upload failed. Please try uploading fewer files or check file formats.'
    });
  }
};
  ...
  try {
    ...
    const patientData = {
      ...
      familyHistory: {
        ...
        hypertension:
          confirmedData.familyHistoryHTM === 'yes' || confirmedData.familyHistoryHTM === true,
      ...
      pastHistory: {
        ...
        cancer:
          confirmedData.pastHistoryCancer === 'yes' || confirmedData.pastHistoryCancer === true,
    ...
    if (healthMetrics.homaIR) {
      ...
      console.log('⚠️ HOMA-IR not calculated (missing glucose or insulin)');
    }
    if (healthMetrics.tygIndex) {
      console.log(`📈 TYG Index: ${healthMetrics.tygIndex.value} (${healthMetrics.tygIndex.status})`);
    } else {
      console.log('⚠️ TYG Index not calculated (missing triglycerides or glucose)');
    }
    if (healthMetrics.bmi) {
      console.log(`📈 BMI: ${healthMetrics.bmi.value} (${healthMetrics.bmi.status})`);
    } else {
      console.log('⚠️ BMI not calculated (missing weight or height)');
    }
    if (healthMetrics.waistCircumference) {
      console.log(`📈 Waist: ${healthMetrics.waistCircumference.value} cm (${healthMetrics.waistCircumference.status})`);
    } else {
      console.log('⚠️ Waist Circumference not calculated (missing waist value)');
    }
