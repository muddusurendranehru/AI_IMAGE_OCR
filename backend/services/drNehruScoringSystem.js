...
const DOCTOR_INFO = {
    ...
    message: "CONTACT PHYSICIAN METABOLISM SPECIALIST DR MUDDU SURENDRA NEHRU, PROFESSOR OF MEDICINE, 09963721999. www.homahealthcarecenter.in"
};
...
function calculateDrNehruScore(labValues, patientData = {}) {
    ...
    if (labValues.triglycerides && labValues.glucose) {
        const tygIndex = Math.log(parseFloat(labValues.triglycerides) * parseFloat(labValues.glucose) / 2);
        if (tygIndex > 4.5) {
            totalScore += 15;
            details.push({
                parameter: 'TYG Index',
    ...
    if (pastHistory.cad) { pastHistoryPoints += 5; pastConditions.push('CAD'); }
    if (pastHistory.cva) { pastHistoryPoints += 5; pastConditions.push('CVA'); }
    if (pastHistory.cancer) { pastHistoryPoints += 5; pastConditions.push('Cancer'); }
    if (pastHistory.ptca) { pastHistoryPoints += 5; pastConditions.push('PTCA/Stent'); }
