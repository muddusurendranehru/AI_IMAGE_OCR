...
const imagePreprocessing = require('../utils/imagePreprocessing');
  ...
  try {
    ...
    if (!pdfPoppler) {
      ...
      return {
        text:
          data.text ||
          'PDF appears to be image-based but OCR tools not available on this system. Please upload images instead.',
        confidence: 50,
        pages: data.numpages,
    ...
    console.log(
      `✅ OCR complete! ${allText.length} chars, confidence: ${avgConfidence.toFixed(1)}%`
    );
 ...
 * Preprocesses color images to grayscale for better OCR accuracy
 */
async function processWithTesseract(imagePath) {
  try {
    ...
    // Preprocess image: Convert color to grayscale for better OCR
    try {
      processedImagePath = await imagePreprocessing.preprocessImageForOCR(imagePath);
      needsCleanup = processedImagePath !== imagePath; // Only cleanup if we created a new file
    } catch (preprocessError) {
      console.warn('⚠️ Preprocessing skipped, using original image:', preprocessError.message);
    ...
    if (needsCleanup) {
      await imagePreprocessing.cleanupPreprocessedImage(processedImagePath);
    }
...
function fixOCRDecimalErrors(testName, value) {
  ...
  if (
    testName.toLowerCase().includes('c-peptide') ||
    testName.toLowerCase().includes('cpeptide') ||
    testName.toLowerCase().includes('c peptide')
  ) {
...
function extractTestResults(text) {
  ...
  const testPatterns = [
    ...
    {
      pattern:
        /(?:Fasting Blood Sugar|FBS|Fasting Glucose|Blood Glucose|Glucose|Blood Sugar)[:\s]+(\d+\.?\d*)\s*(?:mg\/dL|mg\/dl|mgdl)?/gi,
      key: 'glucose',
    ...
    {
      pattern:
        /(?:Post-lunch Blood Sugar|Post Lunch Blood Sugar|PP Blood Sugar|PPBS|Post Prandial)[:\s]+(\d+\.?\d*)\s*(?:mg\/dL|mg\/dl|mgdl)?/gi,
      key: 'postLunchSugar',
    ...
    {
      pattern:
        /(?:Insulin|Fasting Insulin|Serum Insulin)[:\s]+(\d+\.?\d*)\s*(?:μU\/mL|uU\/mL|mU\/L|International Units|IU)?/gi,
      key: 'insulin',
      name: 'Insulin'
    },
    ...
    {
      pattern:
        /(?:HbA1c|HbA1C|Hb A1c|A1C|Glycated Hemoglobin|Glycosylated Hemoglobin)[:\s]+(\d+\.?\d*)\s*(?:%)?/gi,
      key: 'hba1c',
      name: 'HbA1c'
    },
    ...
    {
      pattern:
        /(?:Total Cholesterol|Cholesterol Total|Cholesterol)[:\s]+(\d+\.?\d*)\s*(?:mg\/dL|mg\/dl)?/gi,
      key: 'cholesterol',
    ...
    {
      pattern:
        /(?:Waist Circumference|Waist|Abdominal Circumference)[:\s]+(\d+\.?\d*)\s*(?:cm|centimeters?|inches?)?/gi,
      key: 'waist',
    ...
    {
      pattern:
        /(?:WBC|White Blood Cell|Leucocyte Count)[:\s]+(\d+\.?\d*)\s*(?:cells\/cumm|\/cmm|x10\^3\/uL)?/gi,
      key: 'wbc',
      name: 'WBC'
    },
    {
